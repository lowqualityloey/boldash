...
const validDone = {
  ...
  must_pass: [
    ...
    { type: 'command', name: 'OAuth callback integration test', run: 'npm test -- --grep oauth', timeout_ms: 900000 },
    { type: 'regex_in_file', name: 'callback registered', path: 'src/auth/index.ts', pattern: 'registerCallback\\(' },
    { type: 'state_check', name: 'risk not critical', check: "state.task.risk != 'critical'" },
...
describe('route.schema.json (AC-1)', () => {
  it('accepts the contract-valid proposal', () => expect(check('route', validRoute)).toBe(true));
  it('rejects unknown top-level keys', () =>
    expect(check('route', { ...validRoute, hacker: true })).toBe(false));
  it('rejects out-of-range level and unknown risk', () => {
    expect(check('route', { ...validRoute, route: { workflow: 'feature', level: 9 } })).toBe(false);
    expect(check('route', { ...validRoute, task: { ...validRoute.task, risk: 'yolo' } })).toBe(false);
  });
});

describe('task.schema.json (AC-1)', () => {
  it('accepts a minimal proposed task', () => expect(check('task', validTask)).toBe(true));
...
describe('task-state-file.schema.json (AC-1)', () => {
  ...
  it('rejects garbage at any depth', () => {
    expect(check('taskStateFile', { schema_version: 1, tasks: [{ id: 'TASK-001' }] })).toBe(false);
...
describe('done.schema.json (AC-1, AC-4)', () => {
  ...
  it('rejects malformed per-type payloads (command without run; file_exists without path; extras)', () => {
    expect(
      check('doneContract', { task_id: 'T', must_pass: [{ type: 'command', name: 'missing run here' }] }),
    ).toBe(false);
    expect(
      check('doneContract', { task_id: 'T', must_pass: [{ type: 'file_exists', name: 'no path at all' }] }),
    ).toBe(false);
    expect(
      check('doneContract', {
        task_id: 'T',
        must_pass: [{ type: 'file_exists', name: 'sneaky extra key', path: 'x', exploit: 'rm -rf' }],
      }),
    ).toBe(false);
  });
