...
Caching (`--no-cache` is v0.3.0 per ADR 0005), parallel check execution, container isolation of commands (SECURITY.md §1 remains "declared commands run on host"), policy evaluation (MS-3+… Phase 2), interactive review collection (`human_approval` is a *requirement flag*, not a check type).
